...
% setup for funds rate surprises
...
for i = nlags+1:np;
   ...
   x4=x(i-4,:)';
   ee=e(i,:)';
   ee(113) = xptr1*(4*(x1(92)-x2(92))-x1(148));	% pdot updating rule: pitarg shock

%   ee(113) = -zptr1*(x1(1)-rffexp)/infcoef;		% funds surprise updating rule: pitarg shock


   ee(1)   = infcoef*(0.01+x1(148)+ee(113));	% rff shock

   if i <nlags+5
      ee(1) = ee(1)+0;
   end

%   [i,x1(2),rffexp,x1(2)-rffexp,ee(4),x1(3)];

   e(i,:)=ee';
   xc=amat1*x1+amat2*x2+amat3*x3+amat4*x4+bmat*ee;
   x(i,:)=xc';
  rffexp = (1-mruler0)*(xc(92)-x4(92))+mruler0*xc(1)+mrulep0*(xc(92)-x4(92)-xc(148))+mruley0*xc(71)/ybar0;
end;
...
sygap0 = 100*x(4:np,71)/ybar0;
sinf0  = 400*(x(4:np,92)-x(3:np-1,92));
srff0  = 100*x(4:np,1);
spitarg0 = 100*x(4:np,148);

totgap=.25*cumsum(sygap0);
sacrat=totgap./(sinf0+0.000000000001);

t=0:40;
subplot(2,2,1);
plot(t,400*(x(4:44,92)-x(3:43,92)));
title('Price Inflation');xlabel('Quarters');ylabel('Percentage Points');   
...
subplot(2,2,4);  plot(t,sacrat(4:44));
title('Sacrifice Ratio');xlabel('Quarters');ylabel('Percentage Points');
